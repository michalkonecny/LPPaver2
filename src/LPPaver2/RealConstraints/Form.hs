...
formNegComp form =
  case lookupFormNode form form.root of
    FormComp {comp, e1, e2} ->
      -- e.g. not (x ≤ y) is equivalent to y < x, so we flip the order of e1 and e2 and switch the strictness.
      formComp (flipStrictness comp) (Expr {nodes = form.nodesE, root = e2}) (Expr {nodes = form.nodesE, root = e1})
      where
