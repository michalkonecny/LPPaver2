<script setup lang="ts">
  ...
  import { getHexTriangulation, type DomainAndN, type Triangulation2D } from '../formulas/mesh';
  ...
  function getExprValueBounds(e: Expr | undefined): Interval<number[]> | undefined {
  ...
  function makeBoundTraces(e: Expr | undefined, truthValues?: number[]): Trace[] {
    const triangulation = denseTriangulation.value;
  ...
  const boundsTraces = computed<Trace[]>(() => {
    ...
    if (expr.value) {
      ...
      const z = compValues.map((k) => kleeneanSwitch(k, 1, 0, -1));
...
<template>
  ...
  <div class="d-flex align-items-center gap-2 mt-2">
    <!-- values toggle -->
    <label class="form-check-label text-nowrap" for="valuesToggle">Values:</label>
    ...
    <!-- bounds toggle -->
    <label class="form-check-label text-nowrap" for="boundsToggle">Bounds:</label>
    <div class="form-check w-auto">
      <input
