...
*/
  ...
  function setE1Width(w: number) {
    e1Width.value = w;
  }
  function setE2Width(w: number) {
    e2Width.value = w;
  }
    ...
    if (props.form?.f.tag === 'FormComp') {
      if (e1Width.value !== null && e2Width.value !== null) {
        ...
        emitOutInfo({
          ...
          formType:
            comp == 'CompEq'
              ? 'CIE' // == is a conjunction of two inequalities
              : comp == 'CompNeq'
                ? 'DIE' // != is a disjunction of two inequalities
  ...
  function setFOutInfo(info: OutInfo) {
    fOutInfo.value = info;
  }
  ...
  function setF1OutInfo(info: OutInfo) {
    f1OutInfo.value = info;
  }
  function setF2OutInfo(info: OutInfo) {
    f2OutInfo.value = info;
  }
    ...
    if (props.form?.f.tag === 'FormBinary') {
      if (f1OutInfo.value !== null && f2OutInfo.value !== null) {
        ...
        const formType: IEFormType =
          props.form.f.bconn == 'ConnAnd' && f1IsCIE && f2IsCIE
            ? 'CIE' // conjunction of inequalities
            : props.form.f.bconn == 'ConnOr' && f1IsDIE && f2IsDIE
  ...
  function setFcOutInfo(info: OutInfo) {
    fcOutInfo.value = info;
  }
  function setFtOutInfo(info: OutInfo) {
    ftOutInfo.value = info;
  }
  function setFfOutInfo(info: OutInfo) {
    ffOutInfo.value = info;
  }
  ...
  const highlightedSubExpr = computed(() =>
    props.highlightedSubFormExpr?.type === 'expr'
      ? props.highlightedSubFormExpr.exprHash
      : undefined,
  );

  const isTopLevelComparison = computed(
    () => props.form?.f.tag === 'FormComp' && formType.value !== 'Other',
  );
...
<template>
  ...
  <span v-else @click="clickedHere">
    ...
    <span v-else-if="form.f.tag === 'FormFalse'" :style="style">{{
      emitOutInfoFromString('False')
    }}</span>
    <!-- comparisons -->
    <span
      v-else-if="form.f.tag === 'FormComp'"
      :style="style"
      :class="{
        'd-flex': true,
        'flex-column': !binaryCompFitsHorizontal,
    ...
    >
      <span class="px-1">
        <FormattedExpr
          :expr="form.f.e1"
          :widthLimit="binaryCompChildWidthLimit"
          @width="setE1Width"
          @click="(e) => emits('click', { type: 'expr', exprHash: e })"
          :highlightedExpr="highlightedSubExpr"
        />
      </span>
    ...
    <span
      v-else-if="form.f.tag === 'FormUnary'"
      :style="style"
    ...
    <span
      v-else-if="form.f.tag === 'FormBinary'"
      :style="style"
      :class="{
        'd-flex': true,
        'flex-column': !binaryConnFitsHorizontal,
        'align-items-center': true,
      }"
    >
      <span class="px-1">
        <FormattedForm
          :form="form.f.f1"
          :formValues="formValues"
          :inInfo="binaryChildInInfo"
          @outInfo="setF1OutInfo"
          @click="(d) => emits('click', d)"
          :highlightedSubFormExpr="highlightedSubFormExpr"
        />
      </span>
    ...
    <span
      v-else-if="form.f.tag === 'FormIfThenElse'"
      :style="style"
      :class="{
        'd-flex': true,
        'flex-column': !iteFitsHorizontal,
        'align-items-center': true,
      }"
    >
      <span>If</span>
      <span class="px-1">
        <FormattedForm
          :form="form.f.fc"
          :formValues="formValues"
          :inInfo="iteChildInInfo"
          @outInfo="setFcOutInfo"
          @click="(d) => emits('click', d)"
          :highlightedSubFormExpr="highlightedSubFormExpr"
        />
      </span>
